...
<head>
    ...
    <title>Tambah Data Simpan Pinjam</title>
</head>
...
<body>
    ...
    <a href="index.php" class="btn btn-primary"><i class='fa fa-arrow-left'></i> Kembali</a>
        <form action="" method="post">
            <?php
                $sqlGetLastId = "SELECT id FROM simpanp ORDER BY id DESC LIMIT 1";
    ...
    <select class="form-select" name="loker" required>
        <option value="">Pilih Option</option>
        <option value="Pensiunan">Pensiun</option>
        <option value="Telkom">Telkom</option>
        <option value="Security">Security</option>
        <option value="CS">CS</option>
        <option value="Kary Kop">Kary Kop</option>
        <option value="Kary POJ">Kary POJ</option>
...
<div class="mb-3">
  ...
  <input type="number" class="form-control" id="pokok" name="pokok" required onkeyup="formatRupiah(this.value, 'pokok-rupiah')">
  <span id="pokok-rupiah"></span>
  <input type="hidden" id="pokok-hidden" name="pokok-hidden">
</div>
<div class="mb-3">
  <label for="sukarela" class="form-label">Sukarela</label>
  <input type="number" class="form-control" id="sukarela" name="sukarela" required onkeyup="formatRupiah(this.value, 'sukarela-rupiah')">
  <span id="sukarela-rupiah"></span>
  <input type="hidden" id="sukarela-hidden" name="sukarela-hidden">
</div>
<div class="mb-3">
  <label for="wajib" class="form-label">Wajib</label>
  <input type="number" class="form-control" id="wajib" name="wajib" required onkeyup="formatRupiah(this.value, 'wajib-rupiah')">
  <span id="wajib-rupiah"></span>
  <input type="hidden" id="wajib-hidden" name="wajib-hidden">
</div>
<div class="mb-3">
  <label for="kontribusi" class="form-label">Kontribusi</label>
  <input type="number" class="form-control" id="kontribusi" name="kontribusi" required onkeyup="formatRupiah(this.value, 'kontribusi-rupiah')">
  <span id="kontribusi-rupiah"></span>
  <input type="hidden" id="kontribusi-hidden" name="kontribusi-hidden">
</div>
<script>
function formatRupiah(angka, id) {
  ...
  document.getElementById(id.replace('-rupiah','-hidden')).value = angka;
}
</script>
...
<?php
    if ($_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $sqlAdd = "INSERT INTO simpanp (id, nama, loker, pokok, sukarela, wajib, kontribusi) VALUES ('$id', '$nama', '$loker', '$pokok', '$sukarela', '$wajib', '$kontribusi')";
